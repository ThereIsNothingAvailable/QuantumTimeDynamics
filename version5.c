...
int main() {
   ...
   srand(time(NULL));
    ...
    /*printf("Value of vec init");
    for(int i=0;i<totalsize;i++)
    {
       printf("for the index %d: %.2f",i,vec_init[i]);
    }*/
...
void factorization(float complex* upper_h, float complex *krylov, int n, int min_check,int how_often,int m, float* density_initial )
{
   for(int k=0;k<(n+1);k++)
   {
	...
	for(int jj=0; jj<k;jj++){
	  ...
	  for (int i = 0; i < m; i++) {
           ...
           //Pass the created qq matrix to time_evolve to get eigenstates and eigenvalues by using cgeev and then get sigenstates in way you understood
           	int n1 = sqrt(m);
           	printf("Value of n : %d",n1);
		fcomplex qq[n1*n1]; // Submatrix
		 float complex left[n1*n1];
		 float complex right[n1*n1];
		 float complex* left_dag = (float complex*)malloc(sqrt(m)*sqrt(m) * sizeof(float complex));
		 float complex* norm = (float complex*)malloc(sqrt(m)*sqrt(m) * sizeof(float complex));
		 float complex* rho_t0 = (float complex*)malloc(sqrt(m)*sqrt(m)* sizeof(float complex));
		 float complex* vals = (float complex*)malloc(sqrt(m)*sqrt(m)* sizeof(float complex));
		 float complex* left_dag_res = (float complex*)malloc(sqrt(m)*sqrt(m)* sizeof(float complex));
		 float complex* right_res = (float complex*)malloc(sqrt(m)*sqrt(m)* sizeof(float complex));
		
		
		for (int i = 0; i < n1; i++) {
			for (int j = 0; j < n1; j++) {
		    		rho_t0[i*n1+j] = density_initial[i*n1+j];
		    		rho_t0[i*n1+j] = cimag(0);
				}
	    		} 
		
		for (int i = 0; i < n1; i++) {
			for (int j = 0; j < n1; j++) {
		    		qq[i*n1+j].re = creal(result[i*n1+j]);
		    		qq[i*n1+j].im = cimag(result[i*n1+j]);
				}
	    		}    
	    		
	    		
	    	// LAPACK function to compute eigenvalues and eigenvectors
	    	char jobvl = 'V'; // Compute left eigenvectors (N for no, V for yes)
	    	char jobvr = 'V'; // Compute right eigenvectors (N for no, V for yes)
	   
	    	int lda = n1, ldvl = n1, ldvr = n1, info;
	    	fcomplex wkopt;
	    	fcomplex* work;
	    	float rwork[2*n1];
	    	fcomplex w[n1], vl[ldvl*n1], vr[ldvr*n1];

	    	int32_t lwork = -1;
		cgeev_( "Vectors", "Vectors", &n1, qq, &lda, w, vl, &ldvl, vr, &ldvr, &wkopt, &lwork, rwork, &info );

		lwork = (int32_t)(wkopt.re);
		work = (fcomplex*)malloc( lwork*sizeof(fcomplex) );
		/* Solve eigenproblem */
		
		// This is causing an error ** On entry to CGEBAL parameter number  3 had an illegal value
		cgeev_( "Vectors", "Vectors", &n1, qq, &lda, w, vl, &ldvl, vr, &ldvr, work, &lwork, rwork, &info ); //where w is eigenvalues, vl is left eigenvectors, vr is right eigenvectors
		...
		if (jobvl == 'V') {
		    for (int i = 0; i < n1; i++) {
			for (int j = 0; j <n1; j++) {
				left[i*n1+j] = vl[i*n1+j].re;
				left[i*n1+j] = vl[i*n1+j].im;
			}
		    }
		}    
		...
		if (jobvr == 'V') {
		    for (int i = 0; i < n1; i++) {
			for (int j = 0; j <n1; j++) {
				right[i*n1+j] = vr[i*n1+j].re;
				right[i*n1+j] = vr[i*n1+j].im;
			}
		    }
		}
